(() => {
  ...
  const MAP_EXIT_CONFIG = Object.freeze({
    ...
    // R184 LIERBACH arrival: exact lower yellow-arrow footpath from the supplied reference.
    lierbachFromOppenauSpawn: Object.freeze({
      x: 3150,
      y: 6200
    }),

    // R184 LIERBACH -> OPPENAU: same lower footpath continues through the map edge.
    lierbachOppenauSouth: Object.freeze({
      x1: 2500,
      x2: 3850,
      leavePadding: 18
    }),

    // Returning from LIERBACH lands on OPPENAU's established north road,
    // i.e. exactly where OPPENAU -> LIERBACH is entered.
  ...
  const AREA_SIGNS = Object.freeze([
    ...
    {
      id: "lierbach-allerheiligen", mapId: "lierbach", text: "ALLERHEILIGEN",
      x: 2850, y: 2350, direction: "up", glow: "#ffffff",
      trigger: { x1: 2150, y1: 1650, x2: 3650, y2: 2950 }
    },
    {
  ...
  // ------------------------------------------------------------------
  // R184 LIERBACH — precision pass from supplied painted markup.
  // RED / dark outer terrain = hard collision.
  // BLUE river = hard collision except for the WHITE bridge line.
  // River shimmer is clipped to the exact river polygon so it cannot spill onto banks.
  // ------------------------------------------------------------------
  const LIERBACH_TERRAIN = Object.freeze({
    // Left dark hillside bounded by the supplied jagged red line.
    blockedLeft: Object.freeze([
      Object.freeze([0, 0]),
      Object.freeze([1832, 0]),
      Object.freeze([1832, 2056]),
      Object.freeze([1741, 2489]),
      Object.freeze([2358, 2886]),
      Object.freeze([2594, 3292]),
      Object.freeze([1995, 3445]),
      Object.freeze([2104, 3797]),
      Object.freeze([1896, 4040]),
      Object.freeze([2086, 4365]),
      Object.freeze([1805, 4536]),
      Object.freeze([1478, 4897]),
      Object.freeze([1877, 5014]),
      Object.freeze([1587, 5411]),
      Object.freeze([1107, 5204]),
      Object.freeze([689, 5510]),
      Object.freeze([1143, 6827]),
      Object.freeze([0, 6827])
    ]),

    // Upper/right dark hillside/forest bounded by the supplied red contour.
    blockedRight: Object.freeze([
      Object.freeze([1832, 0]),
      Object.freeze([10240, 0]),
      Object.freeze([10240, 3580]),
      Object.freeze([9578, 4176]),
      Object.freeze([9188, 4374]),
      Object.freeze([8689, 4356]),
      Object.freeze([8444, 3932]),
      Object.freeze([8743, 3580]),
      Object.freeze([8671, 3165]),
      Object.freeze([8381, 3156]),
      Object.freeze([8226, 2832]),
      Object.freeze([7873, 2796]),
      Object.freeze([7020, 2525]),
      Object.freeze([6412, 2588]),
      Object.freeze([6204, 2309]),
      Object.freeze([5623, 2327]),
      Object.freeze([5288, 2219]),
      Object.freeze([4716, 1786]),
      Object.freeze([3945, 2056]),
      Object.freeze([1832, 2056])
    ]),
  ...
  const ALLERHEILIGEN_ROUTE = Object.freeze({
    ...
    // R196 WHITE bridge line from CURRENT reference.
    // Stored RIGHT -> LEFT.
    // S / A+S travels LEFT. W / W+D travels back RIGHT.
    // LEFT endpoint NEVER releases. RIGHT endpoint releases only with D / W+D.
    bridgeSnap: Object.freeze([
      Object.freeze([6402,4123]),
      Object.freeze([5138,3929])
    ]),

    // R197 ORANGE lower bridge snap from the supplied reference.
    ...
    // Down = S / S+D / A+S. Up = W / A+W / W+D.
    // Both ends release normally.
    yellowSnap: Object.freeze([
      Object.freeze([7832,2665]),
      Object.freeze([7905,2914]),
      Object.freeze([7905,3182]),
      Object.freeze([7832,3468]),
      Object.freeze([7592,3855])
    ]),

    guidedEngageDistance: 205,

    // R196 EXACT CURRENT PURPLE BORDER from the supplied reference.
    // After the automatic zigzag, normal movement is confined to THIS area only.
    // Axis-separated collision below keeps sliding along the border smooth.
    walkable: Object.freeze([
      Object.freeze([
        [2694,2195],[2971,1983],[3617,2333],[5111,2509],[6503,2370],
        [7149,2075],[7564,1983],[8025,1983],[8366,2121],[8376,2444],
        [8117,2702],[7804,2730],[7915,2988],[7785,3265],[7804,3597],
        [7518,3883],[6734,3698],[6402,4077],[6144,4196],[6531,4326],
        [6697,4667],[6439,4861],[6845,5128],[6780,5405],[6015,5045],
        [5867,5239],[5710,5543],[5581,5783],[5793,6013],[6476,6050],
        [6596,6207],[6559,6816],[7675,6816],[7472,6465],[7352,6244],
        [7186,6032],[7048,5930],[7075,5589],[7287,5386],[7490,5220],
        [7444,5017],[7361,4861],[7269,4667],[7343,4482],[7638,4307],
        [7804,4113],[7841,3892],[7878,3652],[7988,3468],[8071,3191],
        [8127,2942],[8034,2730],[7629,2656],[6808,2656],[6291,2739],
        [5646,2675],[5065,2739],[4539,2675],[4013,2601],[3506,2518],
        [3045,2407],[2694,2389]
      ])
    ]),
  ...
  // ------------------------------------------------------------------
  // R196 ALLERHEILIGEN KLOSTER
  // Current reference match: screenshot x=447,y=52,w=433,h=289 at zoom 0,
  // converted back into MAP 11 world coordinates.
  // ------------------------------------------------------------------
  const ALLERHEILIGEN_KLOSTER = Object.freeze({
    src: "assets/buildings/ALLERHEILIGEN KLOSTER.png",
    left: 3128,
    top: -18,
    width: 3994,
    height: 2665,
  ...
  let allerheiligenKlosterEl = null;

  function createAllerheiligenKloster() {
    if (allerheiligenKlosterEl) return;

    const c = ALLERHEILIGEN_KLOSTER;
    const image = document.createElement("img");
    image.id = "allerheiligen-kloster";
    image.src = encodeURI(c.src);
    image.alt = "";
    image.draggable = false;
    image.style.position = "absolute";
    ...
    image.style.userSelect = "none";
    image.style.zIndex = String(c.zIndex);
    image.style.display = MAP.id === "allerheiligen" ? "" : "none";

    world.appendChild(image);
    allerheiligenKlosterEl = image;
  }

  function updateAllerheiligenKlosterVisibility() {
    if (!allerheiligenKlosterEl) return;
    allerheiligenKlosterEl.style.display =
      MAP.id === "allerheiligen" ? "" : "none";
  }

  // ------------------------------------------------------------------
  // R197 ALLERHEILIGEN visual-only landmarks from supplied reference.
  ...
  const allerheiligenLandmarkEls = [];

  function createAllerheiligenLandmarks() {
    if (allerheiligenLandmarkEls.length) return;

    for (const c of ALLERHEILIGEN_LANDMARKS) {
      const image = document.createElement("img");
      image.id = c.id;
      image.src = encodeURI(c.src);
      image.alt = "";
      ...
      image.style.display = MAP.id === "allerheiligen" ? "" : "none";
      world.appendChild(image);
      allerheiligenLandmarkEls.push(image);
    }
  }

  function updateAllerheiligenLandmarksVisibility() {
    const visible = MAP.id === "allerheiligen" ? "" : "none";
    for (const image of allerheiligenLandmarkEls) image.style.display = visible;
  }

  let allerheiligenZigzagActive = false;
  let allerheiligenZigzagDistance = 0;
  let allerheiligenZigzagMode = null; // "up" | "down" | null
  let allerheiligenRightSnap = null; // R197: "bridge" | "orangeBridge" | "yellow"
  ...
  function allerheiligenRightPath(kind) {
    if (kind === "bridge") return ALLERHEILIGEN_ROUTE.bridgeSnap;
    if (kind === "orangeBridge") return ALLERHEILIGEN_ROUTE.orangeBridgeSnap;
    if (kind === "yellow") return ALLERHEILIGEN_ROUTE.yellowSnap;
    return null;
  }
  ...
  function tryEngageAllerheiligenRightSnap(dx, dy) {
    ...
    // R197 BOTH bridge lines: A/D ALONE.
    // Both paths are stored RIGHT -> LEFT.
    if (dx !== 0) {
      for (const kind of ["bridge", "orangeBridge"]) {
        const path = allerheiligenRightPath(kind);
        const c = closestPointOnBridgePath(playerX, playerY, path);
        if (!c || c.distance > ALLERHEILIGEN_ROUTE.guidedEngageDistance) continue;

        // At the right endpoint, D is the explicit release direction.
        if (c.progress <= 0.035 && dx > 0) continue;

        allerheiligenRightSnap = kind;
        allerheiligenRightDistance = c.pathDistance;
        allerheiligenRightSnapping = true;
        return true;
      }
    }

    // Existing yellow vertical snap remains unchanged.
    if (dy !== 0) {
      const c = closestPointOnBridgePath(
        playerX,
        playerY,
        ALLERHEILIGEN_ROUTE.yellowSnap
      );

      if (
        c &&
        c.distance <= ALLERHEILIGEN_ROUTE.guidedEngageDistance
      ) {
        if (c.progress <= 0.035 && dy < 0) return false;
        if (c.progress >= 0.965 && dy > 0) return false;

        allerheiligenRightSnap = "yellow";
        allerheiligenRightDistance = c.pathDistance;
        allerheiligenRightSnapping = true;
        return true;
      }
  ...
  function moveAlongAllerheiligenRightSnap(dx, dy, deltaSeconds) {
    ...
    const kind = allerheiligenRightSnap;
    const path = allerheiligenRightPath(kind);
    if (!path) {
      allerheiligenRightSnap = null;
      allerheiligenRightSnapping = false;
    ...
    if (kind === "bridge" || kind === "orangeBridge") {
      // R197: BOTH bridge snaps are horizontal-control routes.
      // Stored RIGHT -> LEFT: A = LEFT along line, D = RIGHT along line.
      const dir = dx < 0 ? 1 : dx > 0 ? -1 : 0;
      const atRight = allerheiligenRightDistance <= 0.001;

      // ONLY the right side releases. D at the right endpoint exits.
      if (atRight && dx > 0) {
        allerheiligenRightSnap = null;
        allerheiligenRightSnapping = false;
        allerheiligenSnapReleaseUntil = performance.now() + 320;
        playerX += 24;
        return false;
      }
      ...
      const nextDistance = Math.max(
        ...
        Math.min(
          metrics.total,
          allerheiligenRightDistance +
            dir * currentPlayerMoveSpeed() * deltaSeconds
        )
      );

      allerheiligenRightDistance = nextDistance;
      const q = pointAtBridgeDistance(path, nextDistance);
      playerX = q.x;
      playerY = q.y;
      facing = dir > 0 ? "left" : "right";

      // LEFT endpoint NEVER releases. D walks back to the right.
      return true;
    }

    // YELLOW line: standard W/S route, diagonals allowed.
    ...
    playerX = q.x;
    playerY = q.y;
    facing = dir > 0 ? "down" : "up";

    const atStart = nextDistance <= 0.001 && dir < 0;
    const atEnd =
  ...
  function isLierbachOuterTerrainBlockedFootPoint(x, y) {
    if (MAP.id !== "lierbach") return false;
    return (
      worldPointInPolygon(x, y, LIERBACH_TERRAIN.blockedLeft) ||
      worldPointInPolygon(x, y, LIERBACH_TERRAIN.blockedRight)
    );
  }

  function lierbachPointInsideBridgeCorridor(x, y) {
    const closest = closestPointOnBridgePath(x, y, LIERBACH_TERRAIN.bridgePath);
    return !!(
      closest &&
      closest.distance <= LIERBACH_TERRAIN.bridgeRiverClearance &&
      closest.progress >= -0.02 &&
      closest.progress <= 1.02
    );
  }
  ...
  function lierbachSvgPath(points) {
    if (!points.length) return "";
  ...
  function canMoveFootTo(x, y) {
    ...
    const inLierbachOppenauSouthExit =
      MAP.id === "lierbach" &&
      x >= MAP_EXIT_CONFIG.lierbachOppenauSouth.x1 &&
      x <= MAP_EXIT_CONFIG.lierbachOppenauSouth.x2;
    ...
    if (y > maxY) {
      ...
      const lierbachOppenauSouthAllowed =
        inLierbachOppenauSouthExit &&
        y <= MAP.height + MAP_EXIT_CONFIG.lierbachOppenauSouth.leavePadding + 80;
    ...
    // R184 LIERBACH: supplied red/dark terrain is hard collision.
    if (isLierbachOuterTerrainBlockedFootPoint(x, y)) return false;
  ...
  function tryEngageLierbachBridge(dx, dy) {
    if (MAP.id !== "lierbach") return false;

    // A/D is authoritative. Diagonals W+D and A+S naturally carry the same
    // horizontal sign and therefore use the same bridge direction.
    const horizontalDirection = dx > 0 ? 1 : dx < 0 ? -1 : 0;
    if (!horizontalDirection) return false;
    ...
    const closest = closestPointOnBridgePath(
      playerX,
      playerY,
      LIERBACH_TERRAIN.bridgePath
    );

    if (!closest || closest.distance > LIERBACH_TERRAIN.bridgeEngageDistance) {
      return false;
    }

    // At either white-line endpoint, outward input must release immediately.
    if (closest.progress <= 0.035 && horizontalDirection < 0) return false;
    if (closest.progress >= 0.965 && horizontalDirection > 0) return false;

    activeBridge = {
      id: "lierbach-wood",
      path: LIERBACH_TERRAIN.bridgePath,
      distance: closest.pathDistance,
      snapping: true
    };
    clearIceVelocity();
  ...
  function movePlayerWithWorldCollision(dx, dy, deltaSeconds) {
    ...
    if (
      MAP.id === "lierbach" &&
      (
        (activeBridge && activeBridge.id === "lierbach-wood") ||
        tryEngageLierbachBridge(dx, dy)
      )
    ) {
  ...
  function updateChurchPlayerDepth() {
    // Map-bound water effects must be hidden before any map-specific early return.
    updateKuhbachCreekEffectVisibility();
    updateLierbachRiverEffectVisibility();
    if (!playerEl) return;
  ...
  function playerInLierbachOppenauSouthExitLane() {
    return (
      MAP.id === "lierbach" &&
      playerX >= MAP_EXIT_CONFIG.lierbachOppenauSouth.x1 &&
      playerX <= MAP_EXIT_CONFIG.lierbachOppenauSouth.x2
    );
  }
  ...
  function checkMapExit() {
    ...
    if (
      playerInLierbachOppenauSouthExitLane() &&
      movingDown &&
      playerY >= MAP.height + MAP_EXIT_CONFIG.lierbachOppenauSouth.leavePadding
    ) {
      switchMap(MAPS.oppenau, MAP_EXIT_CONFIG.oppenauFromLierbachSpawn, true);
      return true;
    }
  ...
  function clampPlayer() {
    ...
    if (southExitOpen) {
      ...
      } else if (MAP.id === "lierbach") {
        leavePadding = MAP_EXIT_CONFIG.lierbachOppenauSouth.leavePadding;
      }
